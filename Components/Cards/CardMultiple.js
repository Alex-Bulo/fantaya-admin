...
import './Card.css';
import { updateCardPointsAndMatch } from '../../callDB';
import EditableCardInfo from './EditableCardInfo';
import { URL } from '../../helper';


function CardMultiple ({ players, teams, fixture, allCaptains }){
    ...
    return(
        fantaStats &&
        <div className='card-container' key={`${teams.home.id}${teams.away.id}`}>
            { editableInfo &&
                ...
                <tbody className={`Card ${showFullCard?'showFullCard': ''}`}>
                    ...
                    {Array.from({ length: fantaStats.maxPlayers }).map((_, idx) => (
                        <tr key={idx} className="card-row">
                            ...
                            {fantaStats.fantaAway.length && <CardPlayer openCard={handlePlayerClick} player={fantaStats.fantaAway.filter(i=>i.lineup !=='No Convocado')[idx]} fantaHome={false} /> }
                        </tr>
